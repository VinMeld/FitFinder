      ...
      <ul className="list-none sm:flex hidden justify-end items-center flex-1">
        {user ? (
          <>
            ...
            <li
              key="signout"
              className={`font-poppins font-normal cursor-pointer text-[16px] mr-0 ${"text-slate-700"}`}
              onClick={signOut}
            >
              <p>Sign Out</p>
            </li>
          ...
          <a
            key="/"
            className={`font-poppins font-medium cursor-pointer text-[16px] ${"text-slate-400"} ${"mb-0"}`}
            onClick={() => {
              router.push(`/`);
              setActive("Home");
            }}
          ...
          <ul className="list-none flex justify-end items-start flex-1 flex-col">
            {user ? (
              navLinks.map((nav, index) => (
                ...
                >
                  <a href={`#${nav.id}`} className="text-slate-50">
                    {nav.title}
                  </a>
                </li>
              ))
